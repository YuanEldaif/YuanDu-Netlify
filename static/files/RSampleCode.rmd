...
### The key of the success is to use QDA as the final predictive model, which coorperates the normal data distribution of each subset.
...
```
```{r, eval=F}
#3. Build 512 seperate models; In order to save running time, you can sample a few models to see the performance. Here I only choose the last model.
system.time(for (i in subsets) {
  ...
  stattrain2 <- data.frame(summarytools::descr(train2[,!(names(train2) %in% c("EXID","target"))], transpose = TRUE)) 
