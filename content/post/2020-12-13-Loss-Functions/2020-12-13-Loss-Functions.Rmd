...
![](/img/Hinge_expo_log.png)
Loss vs y*f(x); $y= \pm 1$, the prediction is f, with class prediction sign(f)
...
**2. <span style="color:navy">Exponential loss</span>**
...
**3. <span style="color:red">Logistic loss(Binomial Deviance)</span>**
...
| Exponential loss| $\\frac {1}{2} log(1+e^{-Yf(x)})$     | $log\\frac{Pr(Y = 1|x)}{Pr(Y = -1|x)}$              | grows exponentially for negative values,more sensitive to outliers |
| Logistic loss   |  $log(1+e^{-Yf(x)})$                  | $\\frac{1}{2}log\\frac{Pr(Y = 1|x)}{Pr(Y = -1|x)}$   | grows linearly for negative values,less sensitive to outliers  |
"
cat(tabl)
```

**Similarity between loss functions: **

- Hinge loss, Exponential loss, Logistic loss have very similar tails, giving zero penalty to points well inside their margin and linear or exponetial penalty to points on the wrong side adn far away. Squared error loss gives a quadratic penalty and points inside their own margin have a strong influence on othe model as well. 
